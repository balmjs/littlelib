...
#扩展区
  ...
  ajax = (url, options)->
    ...
    xhr.onload = ()-> resolveCallback and resolveCallback.apply(xhr, [if (isObject(options) and options.needJson is false) then xhr.responseText else JSON.parse(xhr.responseText)]);
